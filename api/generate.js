export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(process.env.ARK_BASE_URL || 'https://ark.cn-beijing.volces.com/api/v3/images/generations', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${process.env.ARK_API_KEY}` }, body: JSON.stringify({ model: process.env.ARK_MODEL, prompt, image: [image], size: '1920x1920', response_format: 'url' }) });
